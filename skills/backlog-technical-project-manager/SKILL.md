...
For each task `<taskId>` (example `BACK-123`), use a git worktree:

```bash
mkdir -p ../Backlog.md-copies
worktree="../Backlog.md-copies/backlog-<taskId>"
if [ ! -d "$worktree" ]; then
  git worktree add -b "tasks/<taskId>-<short-slug>" "$worktree" main
fi
cd "$worktree"
```

The worktree shares the parent repo's `node_modules` and git objects — no `git clone` or `bun i` needed.

Branch naming convention: `tasks/<taskId>-<short-slug>`

Cleanup on completion:
```bash
cd "$(git rev-parse --git-common-dir)/.."
git worktree remove "../Backlog.md-copies/backlog-<taskId>"
```
...
Enforce isolation: each worker agent must operate only in its own worktree and task branch, so cross-agent file interference should not occur.
...
2. ~~**Using local-path git remotes in dedicated clones**~~ *(resolved by git worktrees)*
   - No longer applicable. Worktrees inherit the parent repo's remotes,
     so `gh pr create` always sees the correct `origin`.
