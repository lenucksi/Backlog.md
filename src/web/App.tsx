...
function App() {
	...
	const handleDeepLinkEditTask = useCallback(
		(task: Task) => {
			setEditingTask(task);
			setShowModal(true);
			...
			if (pathname === "/" || pathname.startsWith("/board")) {
				...
				navigate(`/board/${cleanId}/${sanitizeUrlTitle(task.title)}${window.location.search}`, {
					state: { fromNavigation: true },
				});
			} else if (pathname.startsWith("/tasks")) {
				const cleanId = task.id.replace("task-", "");
				navigate(`/tasks/${cleanId}/${sanitizeUrlTitle(task.title)}${window.location.search}`, {
					state: { fromNavigation: true },
				});
			}
		},
