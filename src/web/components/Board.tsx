...
const Board: React.FC<BoardProps> = ({
	...
	const filteredTasks = useMemo(() => {
		...
		if (filterAssignee === "__unassigned__") {
			result = result.filter(
				(task) => !task.assignee || task.assignee.length === 0 || task.assignee.every((a) => !a.trim()),
			);
	...
	const toggleLaneCollapse = (laneKey: string) => {
		const effectivelyCollapsed =
			collapsedLanes[laneKey] !== undefined
						...
						{onFiltersChange && (
							<section className="flex flex-wrap items-center gap-3" aria-label="Board filters">
								...
								<LabelFilterDropdown
									...
									onChange={(labels) =>
										onFiltersChange({ assignee: filterAssignee, labels, priority: filterPriority, query: filterQuery })
									}
						...
						chips={[
							...
							...normalizedFilterLabels.map((label) => ({
								...
								onRemove: () =>
									onFiltersChange?.({
										assignee: filterAssignee,
										labels: normalizedFilterLabels.filter((l) => l !== label),
										priority: filterPriority,
										query: filterQuery,
									}),
							})),
							...filterPriority.map((p) => ({
								...
								onRemove: () =>
									onFiltersChange?.({
										assignee: filterAssignee,
										labels: normalizedFilterLabels,
										priority: filterPriority.filter((x) => x !== p),
										query: filterQuery,
									}),
							})),
						]}
								...
								{shouldShowLaneHeaders && (
									...
									>
										<div className="flex items-center gap-3 min-w-0">
											...
											<span className="shrink-0 px-2 py-0.5 text-xs font-medium rounded-circle bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors duration-200">
												{taskCount}
											</span>
										</div>
										<div className="flex items-center gap-2 shrink-0">
											{taskCount > 0 && progress === 100 ? (
												...
												<>
													<div className="w-20 h-1.5 rounded-circle bg-gray-200 dark:bg-gray-700 overflow-hidden">
														<div
