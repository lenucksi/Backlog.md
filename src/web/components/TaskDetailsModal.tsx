...
export const TaskDetailsModal: React.FC<Props> = ({
	...
	const hasMilestoneSelection = (milestoneEntities ?? []).some(
		(milestoneEntity) => milestoneEntity.id === milestoneSelectionValue,
	);
	...
	const parentTaskSuggestions = useMemo(
		() => availableTasks.filter((t) => t.id !== task?.id).map((t) => `${t.id} - ${t.title}`),
						...
						<div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 text-xs text-gray-600 dark:text-gray-300 space-y-1">
							<div>
								...
								<span className="text-gray-700 dark:text-gray-200">
									{formatStoredUtcDateForDisplay(task.createdDate)}
								</span>
							</div>
							{task.updatedDate && (
								<div>
									...
									<span className="text-gray-700 dark:text-gray-200">
										{formatStoredUtcDateForDisplay(task.updatedDate)}
									</span>
								</div>
							)}
							{task.dueDate && (
								<div>
									...
									>
										{formatStoredUtcDateForDisplay(task.dueDate)}
									</span>
								</div>
							)}
							{task.deferDate && (
								<div>
									<span className="font-semibold text-gray-800 dark:text-gray-100">Defer:</span>{" "}
									<span className="text-gray-700 dark:text-gray-200">
										{formatStoredUtcDateForDisplay(task.deferDate)}
									</span>
								</div>
							)}
							{task.completedDate && (
								<div>
									<span className="font-semibold text-gray-800 dark:text-gray-100">Completed:</span>{" "}
									<span className="text-gray-700 dark:text-gray-200">
										{formatStoredUtcDateForDisplay(task.completedDate)}
									</span>
								</div>
							)}
							{task.archivedDate && (
								<div>
									<span className="font-semibold text-gray-800 dark:text-gray-100">Archived:</span>{" "}
									<span className="text-gray-700 dark:text-gray-200">
										{formatStoredUtcDateForDisplay(task.archivedDate)}
									</span>
								</div>
							)}
						</div>
