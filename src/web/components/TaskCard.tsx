...
const TaskCard: React.FC<TaskCardProps> = ({
	...
	const getPriorityBadge = (priority?: string) => {
		switch (priority) {
			case "high":
				return { bg: "bg-red-100 dark:bg-red-900/40", text: "text-red-700 dark:text-red-300", label: "High" };
			case "medium":
				return {
			...
			case "low":
				return { bg: "bg-green-100 dark:bg-green-900/40", text: "text-green-700 dark:text-green-300", label: "Low" };
			default:
				return null;
		}
	};
		...
		<div className="relative">
			...
			{showBranchTooltip && isFromOtherBranch && (
				<div className="absolute -top-12 left-1/2 transform -translate-x-1/2 z-50 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-md shadow-lg whitespace-nowrap">
					...
					<div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 rotate-45 size-2 bg-gray-900 dark:bg-gray-700" />
				</div>
			)}
			...
			>
				...
				<div className="flex items-center justify-between gap-2 mb-1.5">
					<div className="flex items-center gap-1.5 min-w-0">
						<span className="text-xs text-gray-400 dark:text-gray-500 font-mono transition-colors duration-200">
							{task.id}
						</span>
						{task.parentTaskId && (
							<span className="inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-300 rounded-circle">
								Subtask
							</span>
						)}
					</div>
				...
				<h4
					className={`font-semibold text-sm line-clamp-2 transition-colors duration-200 ${
						isFromOtherBranch ? "text-gray-600 dark:text-gray-400" : "text-gray-900 dark:text-gray-100"
					}`}
				>
					{task.title}
				</h4>
