...
export function registerLabelCommand(program: Command): void {
	const labelCmd = program
		.command("label")
		.description("manage backlog labels");
	...
	labelCmd
		...
		.action(async (name: string) => {
			...
			if ((reloaded.labels ?? []).some((l) => l.toLowerCase() === name.toLowerCase())) {
				console.error(`Label already exists: ${name}`);
				process.exit(1);
			}
			reloaded.labels = [...(reloaded.labels ?? []), name].sort((a, b) => a.localeCompare(b));
	...
	labelCmd
		...
		.action(async (oldName: string, newName: string) => {
			...
			const labelIndex = (reloaded.labels ?? []).findIndex(
				(l) => l.toLowerCase() === oldName.toLowerCase(),
			);
			if (labelIndex === -1) {
				console.error(`Label not found: ${oldName}`);
				process.exit(1);
			}
			if ((reloaded.labels ?? []).some((l) => l.toLowerCase() === newName.toLowerCase() && l !== reloaded.labels![labelIndex])) {
				console.error(`Target label already exists: ${newName}`);
				process.exit(1);
			}
			reloaded.labels[labelIndex] = newName;
			reloaded.labels = reloaded.labels.sort((a, b) => a.localeCompare(b));
			await core.filesystem.saveConfig(reloaded);
			...
			for (const doc of docs) {
				...
				if (updatedLabels) {
					await core.editDoc(doc.id, { labels: updatedLabels });
				}
			}
	...
	labelCmd
		...
		.action(async (name: string) => {
			...
			const reloaded = await core.filesystem.loadConfig();
			if (!reloaded) process.exit(1);
			const idx = (reloaded.labels ?? []).findIndex(
