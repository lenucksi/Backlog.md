...
export class LabelHandlers {
	...
	async renameLabel(input: LabelRenameArgs): Promise<CallToolResult> {
		...
		const idx = (config.labels ?? []).findIndex(
			(l) => l.toLowerCase() === input.oldName.toLowerCase(),
		);
		if (idx === -1) {
			return {
		...
		if ((config.labels ?? []).some((l) => l.toLowerCase() === input.newName.toLowerCase() && l !== config.labels![idx])) {
			return {
		...
		return {
			content: [{ type: "text", text: `Renamed label "${input.oldName}" to "${input.newName}" in config and all entities.` }],
		};
	}
	...
	async removeLabel(input: LabelRemoveArgs): Promise<CallToolResult> {
		...
		const idx = (config.labels ?? []).findIndex(
			(l) => l.toLowerCase() === input.name.toLowerCase(),
		);
		if (idx === -1) {
			return {
